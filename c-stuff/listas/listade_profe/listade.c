...
listade cons(listade l, int n) {
    ...
    if (l == NULL) {
        ...
        listade current = l;
        while (current->ant != NULL) current = current->ant;
        current->ant = aux;
        aux->sig = current;
        return l;
    }
...
listade insOrd(listade l, int n) {
    ...
    if (l == NULL) {
        ...
        listade iter = l;
        while (iter->ant != NULL) iter = iter->ant;
        while ((iter->sig != NULL) && (n > iter->dato)) iter = iter->sig;
        if (n < iter->dato) {
            aux->sig = iter;
            aux->ant = iter->ant;
            iter->ant = aux;
            if (aux->ant != NULL) aux->ant->sig = aux;
    ...
    if (l == NULL) {
        ...
        while (current != NULL) {
            if (current->dato == elem) {
                listade ant = current->ant;
                listade sig = current->sig;
                if (l == current) {
                    if (ant != NULL)
                        l = ant;
                    else
